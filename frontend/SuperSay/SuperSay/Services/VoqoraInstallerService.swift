...
/// One-way, user-initiated handoff from frozen SuperSay to current Voqora.
/// It verifies the official Voqora DMG and opens it in Finder. It never
/// replaces, deletes, or mutates either app.
@MainActor
final class VoqoraInstallerService: ObservableObject {
    enum State: Equatable {
        ...
        var message: String? {
            switch self {
            case .idle: nil
            case .resolving: "Finding the current Voqora installer…"
    ...
    enum InstallerError: LocalizedError, Equatable {
        ...
        var errorDescription: String? {
            switch self {
            case .invalidRelease:
                "The latest Voqora release did not contain one verifiable DMG. Use the GitHub page instead."
            case .unexpectedResponse:
                "GitHub did not return the installer correctly. SuperSay still works. Try again later."
            case .fileSizeMismatch, .checksumMismatch:
                "The downloaded installer did not match the official Voqora release, so it was not opened. Please try again."
            case .cannotOpenInstaller:
                "The verified installer was saved, but Finder could not open it. Use the GitHub page instead."
            }
        }
    }
    ...
    func downloadAndOpenLatest() {
        ...
        Task { [weak self] in
            ...
            do {
                ...
                guard NSWorkspace.shared.open(destination) else { throw InstallerError.cannotOpenInstaller }
    ...
    static func artifact(from data: Data) throws -> ReleaseArtifact {
        ...
        return ReleaseArtifact(version: release.tagName.trimmingCharacters(in: CharacterSet(charactersIn: "vV")), name: asset.name, downloadURL: url, byteCount: asset.size, sha256: digest)
    }
    ...
    private static func fetchLatestArtifact() async throws -> ReleaseArtifact {
        ...
        request.setValue("SuperSay legacy handoff", forHTTPHeaderField: "User-Agent")
        let (data, response) = try await URLSession.shared.data(for: request)
        guard let http = response as? HTTPURLResponse, (200..<300).contains(http.statusCode) else { throw InstallerError.unexpectedResponse }
    ...
    static func verify(_ fileURL: URL, matches artifact: ReleaseArtifact) throws {
        ...
        guard Int64(values.fileSize ?? -1) == artifact.byteCount else { throw InstallerError.fileSizeMismatch }
        guard try sha256Hex(of: fileURL) == artifact.sha256 else { throw InstallerError.checksumMismatch }
    }

    private static func persist(_ temporaryURL: URL, named name: String) throws -> URL {
        let caches = try FileManager.default.url(for: .cachesDirectory, in: .userDomainMask, appropriateFor: nil, create: true)
        let directory = caches.appendingPathComponent("SuperSay/Voqora-Installers", isDirectory: true)
