...
enum RuntimeEnvironment {
    static var isRunningTests: Bool {
        ...
        return process.environment["SUPERSAY_TESTING"] == "1"
            || process.environment["XCTestConfigurationFilePath"] != nil
            || process.environment["XCTestBundlePath"] != nil
            || process.arguments.contains { $0.localizedCaseInsensitiveContains("xctest") }
            || process.arguments.contains("-ApplePersistenceIgnoreState")
            || NSClassFromString("XCTestCase") != nil
            || isTestBundleEmbeddedInHostApp
    }

    private static var isTestBundleEmbeddedInHostApp: Bool {
        guard let pluginsURL = Bundle.main.builtInPlugInsURL else { return false }
        return FileManager.default.fileExists(
            atPath: pluginsURL.appendingPathComponent("SuperSayTests.xctest").path
        )
    }
}
